...
- __Dealers / Ramp Operators:__ To perform real-time field verification and intercept non-compliant transactions.
...
1. __Automatic Fill__: Upload `farmer_ic_demo.pdf` from the `demo_assets` folder. 
...
5. __Completion__: Upon successful validation, the screen will display "Station Secured!" . Click "__Go to Dashboard__" to proceed.
...
- Expected Result: The system triggers a red "QUOTA EXCEEDED!" warning. A mandatory "Reason for Audit" must be filled to proceed.
...
1. __Check Queue__: Return to the Dealer Dashboard. You will see the "Pending Sync Queue" count has increased (e.g., "12" pending records).
...
1. __Create Manifest__: Once compliant records are selected, look at the "Lorry Manifest" button in the Hub. It shows a notification badge for the number of selected items.
2. __Review Manifest__: Click the button to see the Consolidated Manifest preview. This summarizzes the total load (e.g., 4.25 MT) and the contributing farmers.
3. __Export DDS__: Click "View Consolidated DDS".
